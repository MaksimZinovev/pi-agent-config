...
## Mistakes to Avoid 
 ...
 1. Untyped event parameters — event.result compiled silently because handler params were any. Always import
 and type event parameters from SDK.
 2. Never assume SDK field names — event.result vs event.content cost 2 test runs. Read types.d.ts before
 writing handler code.
 3. tsc --noEmit is mandatory — Would have caught event.result instantly. Set up tsconfig.json with strict:
 true before writing any code.
 4. Don't trust subagent verification — Worker claimed "zero errors" without actually running tsc. Always
 verify yourself.
 5. Regex must account for Pi's command transformations — Pi adds --root <path> to cx/ck commands.
 \bcx\s+overview\b fails; use \bcx\b.*\boverview\b.
 6. try-catch on all event handlers — Missing try-catch caused silent failures that looked like "step
