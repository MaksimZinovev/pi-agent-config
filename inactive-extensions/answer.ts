// BUG: handleInput() intercepted Tab/Shift+Tab for question navigation
// before the Editor could use them for autocomplete completion.
// Typing /pi-<tab> triggered interview instead of completing the command.
// FIX: delegate Tab/Shift+Tab to editor.handleInput() when
// editor.isShowingAutocomplete() is true (public API on Editor).
// Ref: https://github.com/badlogic/pi-mono/issues/TBD
...
class QnAComponent implements Component {
	...
	handleInput(data: string): void {
		...
		if (matchesKey(data, Key.tab)) {
			// If editor has active autocomplete, let it handle Tab for completion
		...
		if (matchesKey(data, Key.shift("tab"))) {
			// If editor has active autocomplete, let it handle Shift+Tab for selection
			if (this.editor.isShowingAutocomplete()) {
				this.editor.handleInput(data);
				this.invalidate();
				this.tui.requestRender();
